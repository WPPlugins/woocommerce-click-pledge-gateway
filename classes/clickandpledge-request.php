...
class clickandpledge_request {
	...
	function buildXML( $settings, $post, $orderplaced )
	{
		...
		$params = $post;
		$cnpVersion = "2.100.007/WP:v".get_bloginfo('version')."/WooCommerce:v".WC_VERSION;
		...
		$ConnectCampaignAlias=$dom->createElement('ConnectCampaignAlias',$configValues['ConnectCampaignAlias'] );
		...
		$dataitem = $Item['data'];$sinc =1;
			 ...
			 $bundles_total = 0;	
				...
				foreach($Item['bundled_items'] as $bnddt => $bndldata)
				{
					...
					if($optnval != 'no' || $optnval == '')
					{
						...
						if($optntitleval == "")
						{
						    $custom_fields[$keyval][$finc] =  $ibndldata['data']->post->post_title ." - ". $optnquantityval ;
						}
						else
						{
			...
			if(count($othermetadata) > 0) {
				foreach($othermetadata as $keymeta => $valmeta) {				
					if((substr($keymeta, 0, 1) != '_') && ($keymeta != "total_sales"))
			...
			$itemname=$dom->createElement('ItemName',$this->safeString(trim($pdetails->post->post_title.$variationdtls), 100));
			...
			if(isset($Item['variation_id']) && $Item['variation_id'] != 0 && $Item['variation_id'] != '')
			{
				   ...
				   $line_subtotal = $this->number_formatprc($this->get_display_price_cnpcart($variation_pdetails->get_price(),1,$Item['product_id']),2,'.','');
				
			} 
			else {
				   $line_subtotal = $this->number_formatprc($this->get_display_price_cnpcart($pdetails->get_price(),1,$Item['product_id']),2,'.','');
				 }
			if($bundles_total != 0)
			{		
			...
			if(isset($Item['addons']) && is_array($Item['addons'])) {//This will handle 'WooCommerce Product Add-ons' plug-in data price				
				foreach($Item['addons'] as $addons_key => $addons_val) {
					if($addons_val['price'] != '') {
						$name = $addons_val['name'] . ' (' . woocommerce_price( get_product_addon_price_for_display ( $addons_val['price'], $Item, true ) ) . ')';
						$name = strip_tags($name);
					} else {
						$name = $addons_val['name'];
					}
					$custom_fields[$pdetails->post->post_title][] = array($name => $addons_val['value']);
					if($addons_val['price'] != '') {
			...
			if( $sku != '' ) {			
				$sku_code=$dom->createElement('SKU',$this->safeString($sku, 100));
		...
		if( isset($orderplaced->billing_company) && $orderplaced->billing_company != '' ) {
			$custom_fields['Billing Company Name'][] = array('' => $orderplaced->billing_company);
		}
		if( isset($orderplaced->shipping_company) && $orderplaced->shipping_company != '' )	{
			$custom_fields['Shipping Company Name'][] = array('' => $orderplaced->shipping_company);
		}
		if( isset($orderplaced->customer_note) && $orderplaced->customer_note != '' ){
			$custom_fields['Order Notes'][] = array('' => $orderplaced->customer_note);
		}
		...
		if(count($custom_fields) > 0) {
			...
			foreach($custom_fields as $key => $val)	{				
				foreach($val as $v) {
					...
					$fieldname   = $dom->createElement('FieldName',$this->safeString($FieldName, 200));
			   ...
			   if(strpos($key, 'BundledData') !== false)
					{$inc++;
					  for($binc = 1;$binc < count($val);$binc++){$fldvalu .= ", ". $val[$binc];}
					     ...
					     $customfield = $customfieldlist->appendChild($customfield);
						 $fieldname = $dom->createElement('FieldName',$this->safeString($val[0], 200));
						 $fieldname = $customfield->appendChild($fieldname);	
						 $fieldvalue = $dom->createElement('FieldValue',$this->safeString($fldval, 500));
						 $fieldvalue = $customfield->appendChild($fieldvalue);
					} 	
			}
		...
		if($post['cnp_payment_method_selection'] == 'CreditCard') {
			if($orderplaced->order_total == 0) {
				//if($settings['Preauthorization'] == 'yes') {
		...
		if(isset($orderplaced->order_shipping)) {	
			...
			if( isset($params['clickandpledge_isRecurring']) &&  $params['clickandpledge_isRecurring'] == 'Recurring' ) {
				if($params['clickandpledge_RecurringMethod'] == 'Installment') {
				$ShippingValue = $this->number_formatprc(($orderplaced->order_shipping/$params['clickandpledge_Installment']), 2, '.', '')*100;
				$shipping_value = $dom->createElement('ShippingValue', $ShippingValue);
				$shipping_value=$shipping->appendChild($shipping_value);
				$ShippingValueCalculate += $this->number_formatprc(($orderplaced->order_shipping/$params['clickandpledge_Installment']), 2, '.', '');
				} else {
				$shipping_value = $dom->createElement('ShippingValue',$this->number_formatprc($orderplaced->order_shipping, 2, '.', '')*100);
				$shipping_value=$shipping->appendChild($shipping_value);
				$ShippingValueCalculate += $this->number_formatprc($orderplaced->order_shipping, 2, '.', '');
				}
			} else {
			$shipping_value = $dom->createElement('ShippingValue',$this->number_formatprc($orderplaced->order_shipping, 2, '.', '')*100);
			$shipping_value=$shipping->appendChild($shipping_value);
		...
		if($tax_display_mode != "incl"){
		   ...
		   {
		    $cart_discount = $orderplaced->cart_discount +  $orderplaced->cart_discount_tax + $UnitTaxCalculate;
		   }
		}
		...
		//echo $UnitTaxCalculate; echo $ShippingTaxCalculate;
		if( isset($params['clickandpledge_isRecurring']) &&  $params['clickandpledge_isRecurring'] == 'Recurring' ) {
		...
		if($orderplaced->needs_shipping_address()) {		
			if( isset($params['clickandpledge_isRecurring']) &&  $params['clickandpledge_isRecurring'] == 'Recurring' ) {
				if($params['clickandpledge_RecurringMethod'] == 'Installment') {
				$TotalShipping = $this->number_formatprc(($orderplaced->order_shipping/$params['clickandpledge_Installment']), 2, '.', '')*100;
				$total_ship=$dom->createElement('TotalShipping', $TotalShipping);
				$total_ship=$trans_totals->appendChild($total_ship);
				} else {
				$total_ship=$dom->createElement('TotalShipping',$this->number_formatprc($orderplaced->order_shipping, 2, '.', '')*100);
				$total_ship=$trans_totals->appendChild($total_ship);
				}
			} else {
			$total_ship=$dom->createElement('TotalShipping',$this->number_formatprc($orderplaced->order_shipping, 2, '.', '')*100);
	  ...
	  if(isset($item['stamp']))
	  {
         ...
         $price = ($price1 + $bndltax) / max( 1, $item['quantity'] );
