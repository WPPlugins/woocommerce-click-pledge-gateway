...
<b>Modified code</b>: return apply_filters( \'woocommerce_cart_needs_payment\', $this->total >= 0, $this );<br>This code change has to be made after each upgrade of WooCommerce. <td><tr>-->
	...
	public function save_acceptedcreditcards_details() {
		...
		function payment_fields() {			
			...
			<?php
			if(WC()->cart->total == 0 && (isset($this->settings['Preauthorization']) && $this->settings['Preauthorization'] == 'no')){
			...
			if(count($this->Paymentmethods) > 0) {
				...
				} else if(WC()->cart->total == 0 && (isset($this->settings['Preauthorization']) && $this->settings['Preauthorization'] == 'yes') && in_array($this->defaultpayment, array('CreditCard','eCheck'))){
				...
				} else if(WC()->cart->total == 0 && (isset($this->settings['Preauthorization']) && $this->settings['Preauthorization'] == 'no') && in_array($this->defaultpayment, array('CreditCard','eCheck'))){
					/*if(count($this->CustomPayments) > 0) {
						$this->defaultpayment = $this->CustomPayments[0];
					} else {*/
						$this->Paymentmethods['Free'] = 'Free';
						$this->CustomPayments[] = 'Free';
						$this->defaultpayment = 'Free';
						//$this->defaultpayment = $this->CustomPayments[0];
					//}
				}
			 /*    if(WC()->cart->total == 0 ){
				...
				foreach($this->Paymentmethods as $pkey => $pval) {
					...
					if(WC()->cart->total == 0 && (isset($this->settings['Preauthorization']) && $this->settings['Preauthorization'] == 'no') && in_array($pkey, array('CreditCard','eCheck'))){
						if(in_array($this->defaultpayment,array('CreditCard','eCheck'))) {
							if(count($this->CustomPayments) > 0) {
								$this->defaultpayment = $this->CustomPayments[0];
							}
					...
					else if(WC()->cart->total == 0 && (isset($this->settings['Preauthorization']) && $this->settings['Preauthorization'] == 'no') && !in_array($pkey, array('eCheck'))){
						if($pkey == $this->defaultpayment) {
							echo '<input type="radio" id="cnp_payment_method_selection_'.$pkey.'" name="cnp_payment_method_selection" class="cnp_payment_method_selection" onclick="displaysection(this.value);" style="margin: 0 0 0 0;" value="'.$pkey.'" checked>&nbsp<b>'.$pval.'</b>&nbsp;&nbsp;&nbsp;';
						} else {
							echo '<input type="radio" id="cnp_payment_method_selection_'.$pkey.'" name="cnp_payment_method_selection" class="cnp_payment_method_selection" onclick="displaysection(this.value);" style="margin: 0 0 0 0;" value="'.$pkey.'">&nbsp;<b>'.$pval.'</b>&nbsp;&nbsp;&nbsp;';
						}
					} else if(WC()->cart->total == 0 && (isset($this->settings['Preauthorization']) && $this->settings['Preauthorization'] == 'yes') && !in_array($pkey, array('eCheck'))){
						if($pkey == $this->defaultpayment) {
							echo '<input type="radio" id="cnp_payment_method_selection_'.$pkey.'" name="cnp_payment_method_selection" class="cnp_payment_method_selection" onclick="displaysection(this.value);" style="margin: 0 0 0 0;" value="'.$pkey.'" checked>&nbsp<b>'.$pval.'</b>&nbsp;&nbsp;&nbsp;';
						} else {
							echo '<input type="radio" id="cnp_payment_method_selection_'.$pkey.'" name="cnp_payment_method_selection" class="cnp_payment_method_selection" onclick="displaysection(this.value);" style="margin: 0 0 0 0;" value="'.$pkey.'">&nbsp;<b>'.$pval.'</b>&nbsp;&nbsp;&nbsp;';
		   ...
		   .cnpccflow {
			...
			<div style="display:<?php if(($this->defaultpayment != 'CreditCard') && ($this->defaultpayment != 'eCheck') && ($this->defaultpayment != 'Free') ) echo 'block'; else echo 'none';?>;" id="cnp_Custom_div">
			<p class="" style="margin:0 0 10px">&nbsp;</p>
