...
class clickandpledge_request {
	...
	function buildXML( $settings, $post, $orderplaced )
	{
		...
		$params = $post;
		$cnpVersion = "2.000.002/WP:v".get_bloginfo('version')."/WooCommerce:v".WC_VERSION;
		...
		$custom_fields = array();
	  	   ...
	  	   foreach(WC()->cart->get_cart() as $i => $Item) {$variationdtls ="";
				...
				if( isset($params['clickandpledge_isRecurring']) &&  $params['clickandpledge_isRecurring'] == 'Recurring' ) {
					...
					if($params['clickandpledge_RecurringMethod'] == 'Installment') {
						if($tax_display_mode != "incl"){
							...
							$UnitTaxCalculate += ($this->number_format(($get_item_tax/$params['clickandpledge_Installment']),2,'.','')*$Item['quantity']);
						}
	  ...
	  public function get_item_cnptax( $item, $round = false ) {
		...
		foreach($item['line_tax_data'] as $date) {  // go thought the first level
		 ...
		 $price1 =0;
		 for($inc=0;$inc<=count($date_count);$inc++)
		 {
		   $val =4+$inc;
		   $parts = explode('.', $item['line_tax_data']['total'][$val]);
		    if(count($parts) > 1) {
		     $price1 = $price1 + round($item['line_tax_data']['total'][$val],2);
			 }
			 else{
			 $price1 = $price1 + $item['line_tax_data']['total'][$val];}
